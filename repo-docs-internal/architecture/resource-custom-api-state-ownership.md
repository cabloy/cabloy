...
A shared persistence domain may still have a genuinely separate, customer-scoped Web self-service contract. In that case, a dedicated Web state owner is legitimate because it does not compete with the Admin Resource boundary. Keep its operations, DTOs, owner scope, cache domain, pages, and SSR admission behavior explicitly separate; see [Admin Resource and Web Self-Service](../../repo-docs/fullstack/admin-resource-and-web-self-service.md) for the public architecture guide.
