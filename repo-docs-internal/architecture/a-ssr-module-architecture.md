...
For the cross-layer Vona/Zova handoff after `handler.js`, also read `repo-docs-internal/architecture/ssr-vona-zova-boundary-and-call-chain.md`. For request-local `public` / `session` profile behavior, read [SSR Request-Local Profiles](ssr-request-local-profiles.md).

The public SSR docs under `repo-docs/frontend/` explain SSR capabilities and usage patterns.
...
For the source-backed evaluation of this structural-cache/request-local-projection boundary, its freshness implications, current test limitations, and gates for any future caller-specific cache, see [SSR `retrieveMenus` Role-Aware Cache Evaluation](ssr-retrieve-menus-role-aware-cache-evaluation.md). It supplements ADR 0006 without changing its authorization or cache-boundary decision.
...
2. `repo-docs-internal/architecture/ssr-memory-leak-investigation-guide.md`
...
- `repo-docs-internal/architecture/README.md`
- `repo-docs-internal/architecture/ssr-memory-leak-investigation-guide.md`
- `repo-docs-internal/decisions/0003-stop-ssr-leak-fixes-at-runtime-core-boundary.md`
