...
As a Zova route-authoring default, a route without `locale` params should select `ssrProfile: 'session'` when it renders locale-sensitive content. The session profile lets SSR resolve the cookie-backed locale input used by hydration and prevents a default-language server tree from diverging from the browser's selected language. This is not an authentication requirement: an anonymous route should set `requiresAuth: false`, and `session` still does not grant Passport, Site, role, or API authority. Routes with an explicit URL locale, or a deliberately locale-neutral public contract, may use `public` when that exception is intentional and hydration-safe.
