...
Cabloy Start keeps the same high-level workflow pattern, but public Start guidance may use different frontend flavors such as `cabloyStartAdmin` and `cabloyStartWeb`, along with its own edition-specific SSR site baselines and project assets in the licensed private repository.
