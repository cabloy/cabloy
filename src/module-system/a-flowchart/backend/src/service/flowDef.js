module.exports = app => {
  ...
  class FlowDef extends app.Service {
    ...
    async __checkRightFlowChartProcess({ host, user }) {
      let { flowId, flowDefId } = host;
      let flow;
      // flowDefId
      if (!flowDefId) {
        flow = await this.ctx.bean.flow.modelFlowHistory.get({ flowId });
        flowDefId = flow.flowDefId;
      }
      // check read right
      const rightRead = await this.__checkRightRead({ host: { flowDefId }, user });
      // no right
      if (!rightRead) return null;
      // flowDef
      let flowDef;
      if (flow) {
        flowDef = await this.ctx.bean.flowDef.getByKeyAndRevision({
          flowDefKey: flow.flowDefKey,
          flowDefRevision: flow.flowDefRevision,
        });
      } else {
        flowDef = await this.ctx.bean.flowDef.getById({ flowDefId });
      }
