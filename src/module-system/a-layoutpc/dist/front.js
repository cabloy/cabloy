    ...
    try {
      var _loop = function _loop() {
        ...
        children.push(c('eb-link', {
          ...
          key: group.id,
          attrs: {
            tabLink: "#".concat(group.id)
          },
          props: {
            onPerform: function onPerform() {
              _this.$f7.tab.show("#".concat(group.id));
            }
          }
        }, [text, close]));
      };
    ...
    if (this.groups.length > 2) {
      children.push(c('f7-link', {
        props: {
          ...
          popoverOpen: "#".concat(this.popoverId)
        }
      }));
    }
...
/* harmony default export */ var headervue_type_script_lang_js_ = ({
  ...
  render: function render(c) {
    ...
    return c('div', {
      staticClass: 'eb-layout-header'
    }, [title, tabs, buttons, mine]);
  },
  computed: {
    ...
    if (this.views) {
      for (var index = 0; index < this.views.length; index++) {
        ...
        children.push(c('eb-view', {
          ...
          id: view.id,
          key: view.id,
          staticClass: "eb-layout-view eb-layout-view-size-".concat(view.size),
          attrs: _viewAttrs,
          props: {
            size: view.size,
  ...
  methods: {
    onViewReady: function onViewReady(view) {
      ...
      this.$meta.vueLayout._patchRouter.loadRoute(_view.url, function (route) {
        if (!route) throw new Error("not found route: ".concat(_view.url));
    ...
    try {
      for (var _iterator = this.groups[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
        ...
        children.push(c('f7-tab', {
          key: group.id,
          staticClass: "eb-layout-group".concat(group.dashboard ? ' eb-layout-group-dashboard' : ''),
          attrs: {
            id: group.id,
...
/* harmony default export */ var layoutvue_type_script_lang_js_ = ({
  ...
  render: function render(c) {
    var header = c('eb-header', {
      ref: 'header',
      style: {
        height: "".concat(this.size.top, "px")
      }
    });
    var groups = c('eb-groups', {
      ref: 'groups',
      style: {
        height: "".concat(this.size.height - this.size.top - this.size.spacing * 2, "px"),
        top: "".concat(this.size.spacing, "px")
      }
    });
    return c('div', {
      staticClass: 'eb-layout-container eb-layout-container-pc'
    }, [header, groups]);
  },
  data: function data() {
    return {
  ...
  methods: {
    ...
    getView: function getView(_ref2) {
      ...
      return new Promise(function (resolve) {
        ...
        if (group.url === url && group.views.length > 0) {
          _this4.$f7.tab.show("#".concat(group.id));
    ...
    removeGroup: function removeGroup(groupId) {
      ...
      var groupCurrent = this.groups[index];
      var groupIdNext;
      ...
      this.$nextTick(function () {
        if (groupIdNext) {
          _this5.$f7.tab.show("#".concat(groupIdNext));
        }

        if (_this5.groups.length === 0 && !groupCurrent.dashboard) {
          _this5.openHome();
        }
      });
    },
...
var css_module = __webpack_require__(1);
