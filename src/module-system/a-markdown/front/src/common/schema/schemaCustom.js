      ...
      parseDOM: [
        {
          ...
          getAttrs: node => {
            const className = String(node.className);
            if (!className.includes('markdown-it-cabloy-block')) return false;
            const params = node.getAttribute('data-block-params');
            const content = decodeURIComponent(node.getAttribute('data-block-content'));
