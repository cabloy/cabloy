...
(function (mod) {
  ...
  function drawConnectorsForChunk(dv, chunk, sTopOrig, sTopEdit, w) {
    ...
    if (dv.copyButtons) {
      ...
      copy.chunk = chunk;
      copy.style.top =
        (chunk.origTo > chunk.origFrom ? top : dv.edit.heightAtLine(chunk.editFrom, 'local') - sTopEdit) + 'px';
  ...
  function getChunks(diff) {
    ...
    for (var i = 0; i < diff.length; ++i) {
      ...
      if (tp == DIFF_EQUAL) {
        ...
        if (cleanToEdit > cleanFromEdit) {
          if (i)
            chunks.push({ origFrom: startOrig, origTo: cleanFromOrig, editFrom: startEdit, editTo: cleanFromEdit });
    ...
    if (startEdit <= edit.line || startOrig <= orig.line)
      chunks.push({ origFrom: startOrig, origTo: orig.line + 1, editFrom: startEdit, editTo: edit.line + 1 });
    return chunks;
  }
