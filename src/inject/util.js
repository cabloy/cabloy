...
* @Last Modified time: 2017-09-12 21:22:58
*/

export default {

  importCSS(moduleInfo, cb) {
      import('../../build/__module/' + moduleInfo.fullName + '/dist/front.min.css').then(() => {
        return cb(null);
      }).catch(e => {
        return cb(e);
      });
  ...
  importJS(moduleInfo, cb) {
      import('../../build/__module/' + moduleInfo.fullName + '/dist/front.min.js').then(m => {
        return cb(null, m);
      }).catch(() => {
