...
4. OAuth-only 用户的合格验证邮箱、禁止任意 recipient 和设置首个密码资格；
5. `password-set` 的用途隔离、短时、单次、重放/过期/竞争安全；
6. 安全审计、限流和密码/token/身份数据红线；
7. Web public SSR 匿名 shell 和 hydration 等价；
...
未登录忘记密码、email/mobile 绑定验证、MFA、设备/会话管理 UI、OAuth 账号管理、管理员编辑他人账户和任意外部头像 URL 均不在本期 ATP 范围。
...
| SSR/document tests            | Web 匿名 HTML、hydration 等价、Admin admission/cache 和 token 页面安全 | matching Home/site owner tests 与 Basic E2E HTTP/browser checks     |
| Browser acceptance            | Web/Admin 导航、共用页面、独立区块、用户旅程和 locale                  | `e2e/specs/cabloy-basic/` 的后续 `@account` 场景                    |
...
- 当前账户 B：OAuth-only 且有合格、已验证、合成 email 的用户；
- 当前账户 C：OAuth-only 且无合格验证 email 的用户；
- 另一个同实例用户和必要时不同实例用户，用于所有权/隔离否定测试；
- Admin Site 准入用户及 Web Site 准入用户；
- 有效、过期、已消费、错误 purpose、被覆盖和竞争消费的合成 `password-set` token 状态；
...
| `ATP-HUA-SET-01`  | 有合格验证邮箱的 OAuth-only 当前账户可按 server-derived recipient 请求 `password-set`；浏览器不能提交任意 email/provider/verified 状态；已有 `auth-simple` 走改密而非 set-link。                    | `PRD-SET-01`, `PRD-SET-02`, `PRD-SET-03`; `SRS-SET-01`–`SRS-SET-03`; `WBS-HUA-40-01`    |
| `ATP-HUA-SET-02`  | 无合格验证邮箱的 OAuth-only 用户得到不可用结果，不发邮件、不建 token；页面只显示所批准的脱敏解释。                                                                                                  | `PRD-SET-03`; `SRS-SET-02`; `WBS-HUA-40-01`                                             |
| `ATP-HUA-TOK-01`  | 无 token、malformed、expired、revoked、superseded、wrong-purpose、wrong-path 和 replay token 安全失败；有效 token 至多由一个并发 caller 成功消费并创建首个凭据。                                    | `PRD-SET-04`–`PRD-SET-05`, `PRD-SEC-02`; `SRS-TOK-*`; `WBS-HUA-40-02`, `WBS-HUA-50-*`   |
| `ATP-HUA-AUD-01`  | 改密、签发链接、消费 token 与拒绝结果产生要求的审计；响应、日志、Passport、截图和证据不含密码/hash/raw token/未批准敏感资料。                                                                       | `PRD-SEC-03`; `SRS-AUD-*`, `SRS-PWD-04`; `WBS-HUA-30-02`, `WBS-HUA-50-02`               |
| `ATP-HUA-RATE-01` | issue/consume 操作执行已批准的限流与滥用策略；拒绝不跨用户/实例泄露状态，也不创建意外 token/副作用。                                                                                                | `PRD-SEC-01`; `SRS-SET-03`, `SRS-AUD-02`; `WBS-HUA-30-02`, `WBS-HUA-40-01`              |
| `ATP-HUA-SSR-01`  | Web public SSR 的账户 route 返回无私有 profile/能力/email/password 信息的匿名 shell；hydration 初树等价，私有加载在 browser admission 后发生；redirect、Site denial 和 direct API denial 分别证明。 | `PRD-UX-03`; `SRS-SSR-01`; `WBS-HUA-60-02`                                              |
...
| `ATP-HUA-SSR-03`  | 公开 password-set 页的 server/hydration 首渲不泄露 token 或预填密码；公开 route 不绕过 token/密码/审计检查。                                                                                        | `PRD-SET-04`, `PRD-SEC-02`; `SRS-TOK-03`, `SRS-SSR-03`; `WBS-HUA-50-01`                 |
...
| `ATP-HUA-SCP-01`  | 未登录 password-reset/forgot-password 仍不在本期可达路径；现有未完成 reset listener 没有被误声明为交付能力。                                                                                        | PRD deferred scope; ADR 0001; `WBS-HUA-80-01`                                           |
...
npm run format
npm run test
npm run test:e2e:basic:clean -- --grep @account
```
...
- future password-reset 仍保持独立范围，除非另有批准的产品和技术记录。
