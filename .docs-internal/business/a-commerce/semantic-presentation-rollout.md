...
Status: in-progress; Product CI-backed sub-slice is complete and unlocks SKU scene/source work, while WBS-80-01 remains open for SKU and batch-closing regression evidence
Current revision: `a63ffa931626f24442e5e7495b0723ee56a0fd03`
...
Completed gate and evidence: Gates A/B, local Gates C/E/F, and Product durable Gate G are complete. Product View/List now project exactly `category: { id, name }`; Create/Update membership and `title`/`createdAt` filters remain unchanged; Category and publication use the approved shared renderers. The tagged Product Admin browser scenario passes locally and in the Commerce CI baseline. At the same revision, SQLite and PostgreSQL CI each passed the full 254-test suite, including `productPresentation.test.ts`, and Vona TypeScript passed on rerun attempt 2.
Commands passed: local focused metadata/runtime test (2 tests); `npm run vona :tools:metadata commerce-catalog`; `npm run build:zova:commerce:admin`; `npm run deps:vona`; `npm run tsc`; local Product/Category catalogue regression (4 tests); `npm run test:e2e:commerce:clean -- --grep @product` (1 browser test); CI SQLite/PG full suites (254 tests each); Commerce paired artifact build/dependency sync/clean browser baseline (16 tests); Vona TypeScript CI `tsc` (rerun attempt 2); `git diff --check`.
Commands still required: SKU scene authority and renderer decision, followed by the serial SKU source/metadata contract loop. Product CI closure is retained and unlocks SKU source work.
Known blocker / decision needed: no Product source or runtime blocker. `WBS-80-01` remains in-progress until SKU and batch-closing regression evidence are retained.
Next single action: begin SKU Gate A scene-authority and Gate B renderer-decision documentation; do not start SKU source work until those gates are recorded.
```
