...
Status: in-progress; Product Gates A/B are documentation-complete; no Product source, metadata, runtime, or CI evidence exists yet
Current revision: b6ae75c173a60e9f2c7e985466a7700ddf5259bd plus this Product authority documentation update
Working-tree classification: baseline contains published Category source of truth, generated output, tests, and retained CI evidence; current WBS documentation records Product scene authority and renderer decisions only. No Product source or generated output changed.
...
Vona source-of-truth paths: future Product work targets the `commerce-catalog` Product entity, model, Create/Update/View/Select DTOs, service, locales, and focused presentation test; generic Zova `ModelResource` and Resource pages remain the state/page owner.
Expected generated and Zova handoff: no generated output or Zova artifact changed for Gates A/B. Gate C must update Product Vona source truth first, use normal metadata/OpenAPI generation, build Commerce Admin SSR/REST artifacts, then run `deps:vona` without hand edits.
Completed gate and evidence: Gates A/B are recorded in the [presentation-contract matrix](./presentation-contracts.md#phase-80-baseline-inventory-and-authority-gates) and confirmed against the current Product and shared Basic renderer source. Category retained CI evidence closes the predecessor resource gate; it does not constitute Product runtime evidence.
Commands passed: documentation-only focused source/renderer inspection and `git diff --check`.
Commands still required: focused Product emitted-metadata/runtime test; normal Product metadata/OpenAPI generation; `npm run build:zova:commerce:admin`; `npm run deps:vona`; `npm run tsc`; tagged Product Admin clean browser scenario; relevant catalogue regression; retained Product CI evidence before SKU source work.
Known blocker / decision needed: no Gate A/B blocker. Product View/List currently lack the approved `category: { id, name }` projection, so Gate C must establish that contract before resource-picker renderer metadata references it. `WBS-80-01` remains in-progress until Product, SKU, and batch closure complete.
Next single action: implement and prove the Product Vona contract under this approved scene authority, beginning with the readonly Category projection and focused metadata/runtime coverage.
```
