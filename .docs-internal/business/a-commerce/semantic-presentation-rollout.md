...
Information areas and field order: Balance identity/SKU; on-hand/reserved/available counters; record dates on View. Audit identity/references; operation/delta/reason/correlation; prior and resulting counters; record dates on View.
Editable / readonly / derived / hidden / snapshot / server-authority boundaries: all Resource List/View fields are readonly; `available` is server-derived; Audit before/after facts are immutable history; `iid` and `deleted` are absent from emitted/runtime responses. Generic Create/Update/Delete and Submit actions remain inactive. `adjustStock` remains a separately authorized command; server-side `systemAdmin`, active-instance scope, nonnegative counters, serializable/retry transaction, and audit append behavior remain authoritative.
Shared/custom renderer decision: raw `skuId`, default integer/text/operation rendering, and existing shared date/date-range renderers are sufficient; no reverse-chain custom renderer prerequisite was needed.
Vona source-of-truth paths: `commerce-trade/src/dto/stockBalanceSelectResItem.tsx`; `stockBalanceView.tsx`; `stockAuditSelectResItem.tsx`; `stockAuditView.tsx`; `service/stockBalance.ts`; `service/stockAudit.ts`; and `commerce-trade/test/stockPresentation.test.ts`.
