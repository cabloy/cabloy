...
Webhook endpoints are `@Passport.public()` only to bypass end-user Passport. The ingress route is `POST /pay/webhook/:providerName/:clientName`: its route names select an enabled Provider and a declared client, but are not credentials. Provider-specific signature verification, verified event identity, and post-verification PaymentSession binding are the authentication boundary.

The controller must use `ctx.request.rawBody`; it must not reserialize parsed JSON for signature verification. Normal Vona instance resolution establishes the request's active instance before the controller runs. The route resolves the selected client's private Provider options, requires that client to support webhooks, and passes the raw body, parsed body, and headers to `verifyWebhook()`. Provider-specific verification authenticates the request using those selected options; unsigned request data cannot override a selected client secret or a persisted PaymentSession binding. After verification, `a-pay` requires provider, client, environment, amount, and currency to match a PaymentSession in the active instance before trusted mutation. The verified receipt is stored in `WebhookInbox`, deduplicated by active instance/provider/client/environment/event ID and payload hash, normalized, and then causes atomic session/audit/outbox updates. The current `pay-mock` verifier uses the selected client's `secretWebhook` and accepts only signed terminal payment facts; it sends its internal webhook through this route with the active instance selector. Live-provider merchant/account validation remains the responsibility of the Provider verifier.
