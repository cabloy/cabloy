...
export class EntityStockAudit extends EntityBase {
  ...
  @Api.field(v.title($locale('Delta')), v.required(), z.int(), ZovaRender.order(3))
  delta: number;

  @Api.field(v.title($locale('Reason')), v.required(), v.min(1), ZovaRender.order(4))
  reason: string;

  @Api.field(v.title($locale('CorrelationId')), v.required(), v.min(1), ZovaRender.order(5))
  correlationId: string;

  @Api.field(v.title($locale('OnHand')), v.required(), z.number().int().min(0), ZovaRender.order(6))
  onHand: number;
