  ...
  async execute(data: IOutboxDispatchJobData, _options?: IQueuePushOptions): Promise<void> {
    ...
    try {
      ...
      if (event.eventType === 'payment.outcome.v1') {
        await payScene.onPaymentOutcome(event.payload as IPaymentOutcomeEvent);
      } else if (event.eventType === 'refund.outcome.v1') {
        await payScene.onRefundOutcome(event.payload as IRefundOutcomeEvent);
      } else {
