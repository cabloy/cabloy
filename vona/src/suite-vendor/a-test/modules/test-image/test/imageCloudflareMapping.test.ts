...
describe('imageCloudflareMapping.test.ts', () => {
  it('action:image:cloudflare mapping', async () => {
    ...
    const clientName = `mapping-${randomUUID()}`;
    ...
    await app.bean.executor.mockCtx(
      async () => {
        const file = path.join(os.tmpdir(), `test-image-cloudflare-${randomUUID()}.txt`);
        ...
        try {
          ...
          const finalizedDirectUrl = await app.bean.image.getVariantUrl(
            directUpload.id,
            'original',
          );
          assert.equal(
